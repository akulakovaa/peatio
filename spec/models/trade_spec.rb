...
describe Trade, '#market_ticker_from_influx' do
  ...
  context 'no trades executed yet' do
    it 'returns ticker' do
      expect(Trade.market_ticker_from_influx(market: :btcusd)).to eq([])
    end
  end
  ...
  context 'single trade was executed' do
    ...
    it 'returns ticker' do
      expect(Trade.market_ticker_from_influx(market: trade.market_id).except(:time)).to eq(expected_ticker)
    end
  end
  ...
  context 'multiple trades were executed' do
    ...
    it 'returns ticker' do
      expect(Trade.market_ticker_from_influx(market: trade1.market_id).except(:time)).to eq(expected_ticker)
    end
  end
end
